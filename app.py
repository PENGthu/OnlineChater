...
def personalchat():
    ...
    if form.validate():
        ...
        per_history = PersonalChatLog.query.filter(
            and_(PersonalChatLog.send_by_user_id == receiver, PersonalChatLog.receiver== g.user.username)
        ).all()
        history_show = '聊天记录:\n---下面是由对方发送的消息---\n'
        for per in per_history:
            history_show += ('--' + str(per.send_time) + '-- ' + per.send_by_user_id + ':' + per.text + '\n')
        per_history = PersonalChatLog.query.filter(
           and_(PersonalChatLog.send_by_user_id ==g.user.username, PersonalChatLog.receiver== receiver)
        ).all()
        history_show+='---下面是您发送给对方的消息---\n'
        for per in per_history:
            history_show += ('--' + str(per.send_time) + '-- ' + per.send_by_user_id + ':' + per.text + '\n')
